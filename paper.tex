...
expect {launch_missiles}.to raise_error
    ...
    \item \emph{optional:} Motivate why it's important to integrate \doctest{} with
        Cabal.
...
We propose four combinators, that can be used to set expectations
about the outcome of code examples.

A common expectation is, that a value should be equal to an other
value.
...
HUnit does not provide any versatile mechanism to expect that an
exception is thrown.  So we propose a combinator for that.
