...
\emph{evidence}\/s for that behavior.
For this purpose, 
...
for evidences for users.
To specify evidences for developers,
we make use of Hspec inspired by RSpec~\cite{rspec}
and we proposed some combinators to it.
...
\item
    ...
    code examples (simply ``code'' hereafter) for use with Hspec. (Section \ref{sec:for-developers})
...
A good candidate to describe evidences for users is Doctest in Python community.
...
evidences are evaluated, and failing evidences are reported to developers.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Evidences for users}
\label{sec:for-users}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Evidences for developers}

We propose following extensions for Hspec to 
describe evidences for developers.
