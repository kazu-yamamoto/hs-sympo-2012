...
\begin{quote}\begin{itemize}
    \item
        BDD tools for other programming languages use code examples as
        evidence for behavior.
    \item
        When we apply BDD to Haskell we want to use properties and
        examples to provide evidence for behavior.
    \item
        Hspec already has support for both.
    \item
...
is similar to a specification.  All facets of the system are described
with examples and properties, including every possible corner case.
...
sense is self-evident.  So we do not want to include any examples or
behaviors for that into user documentation, but we still want to be
able to automatically test it.

We propose to use Doctest to test evidence that is useful as user
documentation, and Hspec for evidence that is not useful as user
documentation.
\end{quote}
