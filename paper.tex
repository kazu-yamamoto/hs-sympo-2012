...
example, and the reader is encouraged to imagine a more sophisticated
library, where design decisions might not be as obvious as here.

We present a step-by-step refinement of a single file {\tt Fib.hs}
that contains both implementation and specs.  Each line of code is
only given once when introduced, subsequent code snippets are
...
This first step might appear useless, but as of now we already have
made some choices with regard to the API of our library.  By giving a
usage example, we were forced to think about which arguments our
function takes.  We can now run Doctest to verify that the example
works.
...
We add a specification that forces our function to actually calculate
fibonacci numbers.
...
\noindent Of course, this test now fails when we run it,
...
implementation is quite inefficient, as can be proved by this
specification:
...
But what about negative ones? Our implementation uses the list index
operator {\tt (!!)} which is a partial function that throws an
exeption for negative indexes.  Let's specify that behaviour.
...
\noindent Throwing exceptions from pure code is considered bad
practice, so we guard the {\tt (!!)} operator against negative
indexes.
...
\noindent Now our specification fails, so we adapt it accordingly.
...
\noindent In fact, it is the case that fib returns 0 for all negative
numbers.
...
\noindent This concludes our example coding session.  We have seen how
to utilize behavior driven development to incrementally build a small
library.  The combinators {\tt shouldBe}, {\tt shouldReturn} and {\tt
shouldThrow} can be used to specify behavior of functions in an
intuitive, easy to read manner.  The final version of our library
looks like this.
