...
\begin{itemize}
\item
    We originally implemented Doctest in Haskell and
    extended Haddock markup to support examples, back in 2010
\item
...
\begin{quote}
kazu: I'm uncomfortable with the three paragraphs above. I would change these
as follows:
...
We think that some evidence is useful as user documentation, hence we
want to be able to include it in Haddock comments.  But others are not
...
They should be included in code document. Both kinds of evidence
should be automatically tested.

We propose to use Doctest and Haddock for evidence that is useful as
user documentation, and Hspec for evidence that is not useful as user
documentation. The following subsections disccuss problems when
we try to carry out our proposal of BDD.
\end{quote}

\subsection{Doctest and Haddock}

For Doctest, we should use Haddock markup to express evidence.
Haddock has markup support for code blocks.  And it is widely used to
include properties and examples in documentation.
For instance, the {\tt filepath} package and
the {\tt containers} package adopt this style.
...
\begin{quote}
    ...
    \emph{kazu: My point is that we cannot tell which the author of packages
   want to express, examples or properties.}
\end{quote}
...
\begin{quote}
    \emph{kazu: We should explicitly repeat that we propose to Doctest as a BDD tool here..}
\end{quote}
