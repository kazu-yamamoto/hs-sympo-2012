...
\begin{quote}
\emph{kazu: How implemneted the ``property'' function? Trystan or Simon?}
\end{quote}
...
One problem of the original Hspec is that it still uses HUnit
...
\begin{quote}
    TBD:
\begin{itemize}
    \item RSpec provides a mechanism to expect exceptions.
...
\begin{quote}
\emph{kazu: are parentheses necessary? how about following?}
\begin{verbatim}
  launchMissiles >>=
  (`shouldBe` Left "not implemented")
  launchMissiles `shouldReturn` Left "not implemented"
...
\begin{quote}
    sol: Should we use verify instead of test here?
\end{quote}
