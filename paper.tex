...
\begin{abstract}
We observe that several widely used Haskell libraries do not have
tests.  Test code assists developers in maintaining correctness when
extending software, and shows users how an API is supposed to be used.
To ease the production and enhance the value of test code, we advocate
behavior driven development with two tools: Doctest for user
documentation and Hspec for code documentation.  Our approach can be
used for design, documentation and automated testing.  We hope that
this motivates Haskell programmers to write more and better tests.
\end{abstract}
...
Another example is, that instances of {\tt Monoid} should be
...
instance definitions behave as expected.  Hence
...
of a specification can be useful as \emph{user documentation}. To that end,
...
\item
    We introduce a set of combinators to express \emph{expectations} about
    code examples for use with Hspec
    (Section~\ref{sec:code-doc} and Section~\ref{sec:hspec-comb}).

\item
    We implemented automatic test discovery for Hspec
    (Section~\ref{sec:solution-discovery} and
    Section~\ref{sec:implementation-discovery}).
...
\item[property]
    A property is something testable with
...
\item[spec]
    A spec is similar to a test suite in TDD.  But it is written with
    specification terminology, and produces a specification document
    when run.

\item[user documentation]
    User documentation means the documentation for users of a library.

\item[code documentation]
    Code documentation means a detailed description, or specification,
    of a library.
...
have to solve before we can do so.
...
expectations with that terminology.  The developer has to use
...
And a mechanism for automatic spec discovery is described.
...
GHCi's type defaulting\footnote{%
\url{http://www.haskell.org/ghc/docs/7.4.1/html/users_guide/interactive-evaluation.html#extended-default-rules}
} applies to properties.  If a default type is
...
In this section we describe implementation aspects of the proposed
solution.
...
subsequent code fragments are incremental updates.  Appendix
...
We are grateful to David Waern, Haddock's maintainer, for reviewing
