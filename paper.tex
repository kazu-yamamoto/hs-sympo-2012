...
provide example code, 10 have no tests nor examples).
A lack of tests is not only a quality and maintainability issue.  It
is also a lack of documentation.  Tests document how code is supposed
to be used; and tests documents the behavior of code.
...
\begin{quote}\begin{itemize}
    \item
        sol: Should we state that BDD-style TDD has been pioneered by
        RSpec?  By now I think it would be a good idea, that way it
        feels more natural to the reader that we always look at RSpec.

        @mkl: Where would that fit in?
...
\begin{itemize}
\item
    We originally implemented Doctest, back in 2010.
\item
