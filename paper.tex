...
As discussed in Section~\ref{sec:solution-discovery}, we can use
{\tt hspec-discover} to automatically generate a test driver for Hspec:
...
exception, similar to {\tt shouldThrow} described in Section~\ref{sec:expacting-exceptions}.
...
for code documentation.  For user documentation, two markups for
properties and examples are added to Haddock.  And we implemented
Doctest to verify examples and test properties from Haddock comments.
For code documentation, a set of combinators for expectations is
extended for Hspec.  With these two tools, specification can also be
used for documentation and automatic testing.  We also described a
mechanism that can automatically discover all specs of a project.  We
think that our approach enhances the value of test code, and we hope
